	...
	u16 addr, u8 val)
{
	...
	if(!err)
		dev_info(dev,"abh:96724:w addr(0x%04x)->0x%02x\n",addr,val);
	...
	u16 addr, u32* val)
{
	...
	usleep_range(100, 110);
	if(!err)
		dev_info(dev,"abh:96724:r addr(0x%04x)<-0x%02x\n",addr,(u8)(*val));
	return err;
}

int abh_max96724_lock_link(struct device *dev)
{
...
EXPORT_SYMBOL(abh_max96724_lock_link);

int abh_max96724_unlock_link(struct device *dev)
{
	struct max96724 *priv = NULL;
...
EXPORT_SYMBOL(abh_max96724_unlock_link);

int abh_max96724_monopolize_link(struct device *dev, int link)
{
	struct max96724 *priv = NULL;
	u32 value;
...
EXPORT_SYMBOL(abh_max96724_monopolize_link);

int abh_max96724_enable_link(struct device *dev, int link)
{
    struct max96724 *priv = NULL;
...
EXPORT_SYMBOL(abh_max96724_enable_link);

int abh_max96724_restore_link(struct device *dev)
{
	struct max96724 *priv = NULL;
	u32 value;
...
EXPORT_SYMBOL(abh_max96724_restore_link);

int abh_max96724_check_link_status(struct device *dev, int link)
{
	struct max96724 *priv = NULL;
	CHECKNULL(dev);
	priv = dev_get_drvdata(dev);
...
EXPORT_SYMBOL(abh_max96724_check_link_status);

int abh_max96724_set_link_bandwidth(struct device *dev, int link, int gbps)
{
	struct max96724 *priv = NULL;
	u32 value;
	u16 reg = 0x0010;
...
EXPORT_SYMBOL(abh_max96724_set_link_bandwidth);

static int max96724_setup_pipeline_deepth(struct device *dev,struct gmsl_link_ctx *g_ctx)
{
	struct max96724 *priv = NULL;
	int pipe_id = 0;
	u32 i = 0;
	...
	dev_info(dev,"abh:96724: num_streams=%d\n",g_ctx->num_streams);
	...
	for (i = 0; i < g_ctx->num_streams; i++) {
		...
		max96724_write_reg(dev, (0x40*pipe_id)+max96724_TX45_PIPE_X_DST_CTRL_ADDR+0, (g_ctx->des_link<2)?0x55:0xAA);  // 0x55=controller1  0xAA=controller2
		max96724_write_reg(dev, (0x40*pipe_id)+max96724_TX45_PIPE_X_DST_CTRL_ADDR+1, (g_ctx->des_link<2)?0x01:0x02); 
...
int abh_max96724_start_streaming_deepth(struct device *dev, struct gmsl_link_ctx *g_ctx)
{
	struct max96724 *priv = NULL;
	int err;
	//u32 val;
	...
	dev_info(dev,"abh: 96724 start streaming, link=%c - enter\n",'A'+g_ctx->des_link);

	mutex_lock(&priv->lock);
	err = max96724_setup_pipeline_deepth(dev, g_ctx);
	if (err)
		return err;

	max96724_write_reg(dev,0xf4, 0x0f);
	...
	if(g_ctx->des_link==0 || g_ctx->des_link==1){
        ...
        max96724_write_reg(dev,0x09c3,0x10);
		max96724_write_reg(dev,0x09c3,0x33);
	}

	mutex_unlock(&priv->lock);
	dev_info(dev,"abh: 96724 start streaming - exit\n");
	return 0;
}
EXPORT_SYMBOL(abh_max96724_start_streaming_deepth);
...
int abh_max96724_start_streaming(struct device *dev, struct gmsl_link_ctx *g_ctx)
{
	struct max96724 *priv = NULL;
	int err;
	//u32 val;
...
EXPORT_SYMBOL(abh_max96724_start_streaming);

int abh_max96724_stop_streaming(struct device *dev, struct gmsl_link_ctx *g_ctx)
{
	// struct max96724 *priv = NULL;
	// u32 val;
	// u8 tmp;
	...
	// dev_info(dev,"abh: 96724 stop streaming, link_%c - enter\n",'A'+g_ctx->des_link);
	// mutex_lock(&priv->lock);
	...
	// mutex_unlock(&priv->lock);
	dev_info(dev,"abh: 96724 stop streaming - exit\n");
	return 0;
}
EXPORT_SYMBOL(abh_max96724_stop_streaming);

const struct of_device_id max96724_of_match[] = {
	{ .compatible = "Connor,abh_max96724", },
	{ },
};
...
static int get_mipifreq(void)
{
	...
	if (IS_ERR(file)) {
		printk("abh: no mipifreq define file\n");
		goto defaultval;
	}
	...
	if (ret > 0) {
		buf[ret] = 0;
		printk("abh: read from mipifreq: %s\n", buf);
		freq = simple_strtoul(buf,NULL,10);
		freq = freq / 100;
		if(freq<2 || freq>25){
			printk("abh: mipifreq out of range: %d, use default\n", freq);
			goto defaultval;
		}
		return freq;
	} else {
		printk("abh: failed to read from mipifreq\n");
		goto defaultval;
	}
				...
				const struct i2c_device_id *id)
{
    ...
    dev_info(&client->dev, "abh: [max96724]: probing GMSL Deserializer @0x%x\n",client->addr);

    priv = devm_kzalloc(&client->dev, sizeof(*priv), GFP_KERNEL);
    priv->i2c_client = client;
	priv->regmap = devm_regmap_init_i2c(priv->i2c_client, &max96724_regmap_config);
	...
	if (of_get_property(node, "is-master", NULL)) {
		priv->ismaster = 1;
		dev_info(&client->dev, "abh: this max96724 is master\n");
	}else{
		priv->ismaster = 0;
		dev_info(&client->dev, "abh: this max96724 is slave\n");
	}
	...
	max96724_write_reg(&client->dev,0x98a,0x40); //2 lanes, D-Phy, 2bits VC
	max96724_write_reg(&client->dev,0x9ca,0x40); //2 lanes, D-Phy, 2bits VC
	...
	max96724_write_reg(&client->dev,0xf0,0x02); //pipe1 GMSLB, Z-pipe; pipe0 GMSLA, Z-pipe
	max96724_write_reg(&client->dev,0xf1,0x46); //pipe3 GMSLD, Z-pipe; pipe2 GMSLC, Z-pipe
	max96724_write_reg(&client->dev,0xf4,0x00);
...
MODULE_DESCRIPTION("GMSL Deserializer driver abh_max96724");
